...
const GuardianSection = ({ formData, errors, handleChange, handleBlur, isReadOnly = false }) => {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <InputWrapper label="Mother's Maiden Name" required hasError={!!errors.mothers_maiden_name} errorMessage={errors.mothers_maiden_name}>
                <input name="mothers_maiden_name" value={formData.mothers_maiden_name} onChange={handleChange} onBlur={handleBlur} autoComplete="new-password" placeholder="e.g. Elena Santos" className={inputClasses} disabled={isReadOnly} readOnly={isReadOnly} />
            </InputWrapper>
            <InputWrapper label="Father's Full Name" hasError={!!errors.father_name}>
                <input name="father_name" value={formData.father_name} onChange={handleChange} onBlur={handleBlur} autoComplete="new-password" placeholder="e.g. Roberto Santos" className={inputClasses} disabled={isReadOnly} readOnly={isReadOnly} />
            </InputWrapper>
            <InputWrapper label="Caregiver Relationship" required hasError={!!errors.caregiver_relationship}>
                <select name="caregiver_relationship" value={formData.caregiver_relationship} onChange={handleChange} className={inputClasses} disabled={isReadOnly}>
